...
class ValueConverterScreen extends StatefulWidget {
  const ValueConverterScreen({Key? key}) : super(key: key);
...
class _ValueConverterScreenState extends State<ValueConverterScreen> {
  ...
  void convertDecimalToBytes() {
    ...
    setState(() {
      resultBytes = bytes.map((b) => b.toRadixString(16).padLeft(2, '0')).join(' ');
    });
  }
  ...
  void convertBytesToDecimal() {
    ...
    switch (selectedReprCode) {
      case 68:
        value = CodeReader.readCode(Uint8List.fromList(bytes), 68, bytes.length);
        break;
      case 73:
        value = CodeReader.readCode(Uint8List.fromList(bytes), 73, bytes.length);
        break;
      case 79:
        value = CodeReader.readCode(Uint8List.fromList(bytes), 79, bytes.length);
        break;
      case 65:
